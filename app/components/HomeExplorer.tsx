...
export function HomeExplorer({ items, initialLevel = "all" }: { items: ContentItem[]; initialLevel?: LevelFilter }) {
  ...
  return <main>
    ...
    <section className="category-section"><div className="section-title"><div><span className="eyebrow">QUICK ACCESS</span><h2>{level === "all" ? "你现在想了解什么？" : `${levelLabels[level as StudentLevel]}常用入口`}</h2></div><p>九个常用方向，官方通知与学生经验分开呈现。</p></div><div className="category-grid">{categories.map((item) => <button key={item.slug} className={category === item.slug ? "category-card active" : "category-card"} onClick={() => { setCategory(category === item.slug ? "all" : item.slug); document.getElementById("explore")?.scrollIntoView({ behavior: "smooth" }); }}><span>{item.mark}</span><b>{item.label}</b><small>{item.description}</small><i>→</i></button>)}</div></section>
