...
export function HomeExplorer({ items, mode = "home", initialLevel = "all", initialQuery = "", initialCategory = "all", initialSource = "all" }: HomeExplorerProps) {
  ...
  return <main>
    {mode === "home" && <><section className="hero">
      ...
      <aside className="deadline-panel"><div className="panel-heading"><span>近期截止</span><small>按时间排序</small></div>{deadlineItems.length ? deadlineItems.map((item) => { const days = daysUntil(item.deadlineAt); return <a key={item.id} href={`/item/${item.id}`} className="deadline-row"><div className="deadline-date"><strong>{new Date(item.deadlineAt!).getDate()}</strong><small>{new Date(item.deadlineAt!).getMonth()+1}月</small></div><div><b>{item.title}</b><span>{days === 0 ? "今天截止" : `${days}天后截止`} · {item.audience}</span></div></a>; }) : <div className="empty-mini">暂无临近截止事项</div>}<a className="panel-more" href="/explore">查看全部信息 →</a></aside>
    </section>
    ...
    {mode === "explore" && <section className="explore-section standalone-explore" id="explore"><div className="breadcrumb"><a href="/">首页</a><span>›</span><span>{level === "all" ? "信息广场" : `${levelLabels[level as StudentLevel]}专区`}</span></div><div className="explore-head"><div><span className="eyebrow">INFORMATION DESK</span><h1>{level === "all" ? "信息广场" : `${levelLabels[level as StudentLevel]}信息`}</h1><p>按学生阶段、分类和来源筛选；每条信息均保留来源与核验状态。</p></div><div className="filters"><select aria-label="按学生阶段筛选" value={level} onChange={(e)=>setLevel(e.target.value as LevelFilter)}><option value="all">全部学生</option><option value="undergraduate">本科生</option><option value="graduate">研究生</option></select><select aria-label="按分类筛选" value={category} onChange={(e)=>setCategory(e.target.value)}><option value="all">全部分类</option>{categories.map((item)=><option value={item.slug} key={item.slug}>{item.label}</option>)}</select><select aria-label="按来源筛选" value={source} onChange={(e)=>setSource(e.target.value)}><option value="all">全部来源</option><option value="official">官方渠道</option><option value="campus_org">校内组织</option><option value="external">非官方参考</option><option value="student">学生经验</option></select></div></div>
      ...
      <div className="info-grid">{filtered.map((item) => { const days = daysUntil(item.deadlineAt); const expired = item.validity === "expired" || (days !== null && days < 0); return <article className="info-card" key={item.id}><div className="card-top"><div><span className={`source-pill ${item.sourceType}`}>{sourceLabels[item.sourceType]}</span><span className={`level-pill ${item.studentLevel}`}>{levelLabels[item.studentLevel]}</span></div><span className="category-text">{categoryLabel(item.category)}</span></div><h3><a href={`/item/${item.id}`}>{item.title}</a></h3><p>{item.summary}</p><div className="card-meta"><span>{item.sourceName}</span><span>{item.checkedAt ? `核验 ${item.checkedAt}` : "待核验"}</span></div>{item.validity === "possibly_invalid" && !item.deadlineAt && <div className="deadline-tag expired">信息可能已变化</div>}{item.deadlineAt && <div className={expired ? "deadline-tag expired" : "deadline-tag"}>{expired ? "报名已截止" : `距截止 ${days} 天`}</div>}<a className="card-link" href={`/item/${item.id}`}>查看详情 <span>↗</span></a></article>; })}</div>{filtered.length === 0 && <div className="empty-state"><strong>没有找到匹配内容</strong><p>换个关键词，或清除筛选条件再试试。</p></div>}
    </section>}
  </main>;
}
