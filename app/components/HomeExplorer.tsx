...
export function HomeExplorer({ items, initialLevel = "all" }: { items: ContentItem[]; initialLevel?: LevelFilter }) {
  ...
  return <main>
    <section className="hero">
      ...
      <div className="hero-copy"><span className="eyebrow">中国地质大学（武汉）学生信息平台</span><h1>{heroTitle}，<br/><em>一处查清。</em></h1><p>{heroCopy}</p>
        <div className="audience-switch" aria-label="选择学生阶段"><button className={level === "all" ? "active" : ""} onClick={() => chooseLevel("all")}>全部信息</button><button className={level === "undergraduate" ? "active" : ""} onClick={() => chooseLevel("undergraduate")}><span>本</span>本科生专区</button><button className={level === "graduate" ? "active" : ""} onClick={() => chooseLevel("graduate")}><span>研</span>研究生专区</button></div>
        <div className="hero-search"><label className="sr-only" htmlFor="hero-query">搜索校园信息</label><span aria-hidden="true">⌕</span><input id="hero-query" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={level === "graduate" ? "搜索培养、竞赛或研究生事务…" : "搜索课程、社团、竞赛或升学信息…"}/><a href="#explore">开始查找</a></div>
      ...
      <aside className="deadline-panel"><div className="panel-heading"><span>{level === "undergraduate" ? "本科生近期截止" : level === "graduate" ? "研究生近期截止" : "近期截止"}</span><small>按时间排序</small></div>{deadlineItems.length ? deadlineItems.map((item) => { const days = daysUntil(item.deadlineAt); return <Link key={item.id} href={`/item/${item.id}`} className="deadline-row"><div className="deadline-date"><strong>{new Date(item.deadlineAt!).getDate()}</strong><small>{new Date(item.deadlineAt!).getMonth()+1}月</small></div><div><b>{item.title}</b><span>{days === 0 ? "今天截止" : `${days}天后截止`} · {item.audience}</span></div></Link>; }) : <div className="empty-mini">暂无临近截止事项</div>}<a className="panel-more" href="#explore">查看全部信息 →</a></aside>
    </section>

    <section className="path-section"><Link href="/undergraduate" className="path-card undergraduate"><span className="path-mark">本</span><div><small>UNDERGRADUATE</small><h2>本科生专区</h2><p>选课、课程体验、社团、竞赛、保研与留学</p></div><b>进入专区 →</b></Link><Link href="/graduate" className="path-card graduate"><span className="path-mark">研</span><div><small>GRADUATE</small><h2>研究生专区</h2><p>培养事务、学术活动、实践竞赛与研究生服务</p></div><b>进入专区 →</b></Link></section>

    <section className="category-section"><div className="section-title"><div><span className="eyebrow">QUICK ACCESS</span><h2>{level === "all" ? "你现在想了解什么？" : `${levelLabels[level as StudentLevel]}常用入口`}</h2></div><p>九个常用方向，官方通知、非官方参考与学生经验分开呈现。</p></div><div className="category-grid">{categories.map((item) => <button key={item.slug} className={category === item.slug ? "category-card active" : "category-card"} onClick={() => { setCategory(category === item.slug ? "all" : item.slug); document.getElementById("explore")?.scrollIntoView({ behavior: "smooth" }); }}><span>{item.mark}</span><b>{item.label}</b><small>{item.description}</small><i>→</i></button>)}</div></section>

    <section className="explore-section" id="explore"><div className="explore-head"><div><span className="eyebrow">INFORMATION DESK</span><h2>{level === "all" ? "信息广场" : `${levelLabels[level as StudentLevel]}信息`}</h2></div><div className="filters"><select aria-label="按学生阶段筛选" value={level} onChange={(e)=>setLevel(e.target.value as LevelFilter)}><option value="all">全部学生</option><option value="undergraduate">本科生</option><option value="graduate">研究生</option></select><select aria-label="按分类筛选" value={category} onChange={(e)=>setCategory(e.target.value)}><option value="all">全部分类</option>{categories.map((item)=><option value={item.slug} key={item.slug}>{item.label}</option>)}</select><select aria-label="按来源筛选" value={source} onChange={(e)=>setSource(e.target.value)}><option value="all">全部来源</option><option value="official">官方渠道</option><option value="campus_org">校内组织</option><option value="external">非官方参考</option><option value="student">学生经验</option></select></div></div>
